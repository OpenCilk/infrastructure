...
*Note:* The `-S` flag instructs `clang` to emit human-readable LLVM
...
*Note:* There are other graphs related to LLVM IR other than the CFG
...
*Note:* Depending on the optimizations and the size of the input,
...
code-transformation passes to run to reproduce the issue.
...
*Note:* The `-print-before-all` and `-print-after-all` flags can
